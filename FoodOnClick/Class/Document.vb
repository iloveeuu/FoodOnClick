...
Public Class Document
    ...
    Protected str_riderNRIC As String
    ...
    Public Property riderNRIC() As String
        Get
            riderNRIC = str_riderNRIC
        End Get
        Set(ByVal Value As String)
            str_riderNRIC = Value
        End Set
    ...
    Public Function GetRiderDocument()
        Dim returnMsg As String = "False"
        Dim returnObject As List(Of Document) = New List(Of Document)
        Dim connectionString As String = ConfigurationManager.ConnectionStrings("ConnectionString").ConnectionString
        Dim Query As String = "SELECT useraccount.userid,firstname,lastname,phoneNum,email,document.type,url from document join useraccount on document.userid = useraccount.userid where useraccount.status = 'VETTING' and useraccount.type = 'Rider'  order by document.userid asc"
        Using conn As New SqlConnection(connectionString)
    ...
    Public Function GetCountRiderUser()
        Dim returnInt As Integer = 0
        Dim connectionString As String = ConfigurationManager.ConnectionStrings("ConnectionString").ConnectionString
        Dim Query As String = "Select COUNT(DISTINCT(dbo.document.userid)) As COUNT FROM dbo.document join dbo.UserAccount On dbo.UserAccount.userid=dbo.document.userid where dbo.UserAccount.status='VETTING' and dbo.UserAccount.type = 'Rider'"
        Using conn As New SqlConnection(connectionString)
